...
class Map:
    def __init__(self, map_description_file):
        ...
        self._loadMap(map_description_file)
    ...
    def _loadMap(self, mapContents: str):
        """
        ...
        try:
            ...
            map_contents_lines = mapContents.splitlines()
            header = map_contents_lines[0]
            ...
            for indx, line in enumerate(map_contents_lines[1:]):
    ...
    def _cell2costMatrix(self, cell_x, cell_y, numNeigh):
        """
        Input:
        ...
        Output:
            (connX,connY): 2D coordinates (in the costMatrix!!) \
    ...
    def fillCostMatrix(self, x_end, y_end):
        """
        Fills the cost matrix with the cost of each cell.
        The cost of a cell is the distance to the end point that has a connection.

        :param x_end: final x position
        :param y_end: final y position
        """
        self._initCostMatrix()
        self.costMatrix[x_end][y_end] = 0
        frontier = [(x_end, y_end)]
        ...
        while frontier:
            ...
            for orientation in range(8):
                ...
                if self.costMatrix[cost_coord_x][cost_cord_y] == -2:
    ...
    def _planPath(self, x_ini, y_ini, x_end, y_end):
        ...
        while self.costMatrix[current_position_x][current_position_y] != 0:
            for direction in range(8):
                ...
                (neighbor_x, neighbor_y) = self._cell2costMatrix(current_position_x, current_position_y, direction)
                if self.costMatrix[neighbor_x][neighbor_y] < self.costMatrix[current_position_x][current_position_y]:
                    current_position_x = neighbor_x
                    current_position_y = neighbor_y
                    path.append((current_position_x, current_position_y))
                    break
        return path
