...
from repr import RobotDrawer, Size, Color
from specs import np_close

with description('Repr') as self:
    with it('prints the localization of the robot'):
        spy = Spy()
        drawer = RobotDrawer(printer=spy)
        drawer.draw([0, 0, 0])
    ...
    with it('prints the localization of the robot'):
        spy = Spy()
        drawer = RobotDrawer(printer=spy, color=Color.OTHER)
        drawer.draw([0, 0, 0])
    ...
    with it('prints the localization of the robot'):
        spy = Spy()
        drawer = RobotDrawer(printer=spy, size=Size.BIG)
        drawer.draw([0, 0, 0])
